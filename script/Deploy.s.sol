// SPDX-License-Identifier: UNLICENSED
pragma solidity >=0.8.25 <0.9.0;

import { BaseScript } from "./Base.s.sol";

import { ROPBalanceHolder } from "../src/ROPBalanceHolder.sol";
import { ROPMetadataRenderer } from "../src/ROPMetadataRenderer.sol";
import { RothkoOnPennies } from "../src/RothkoOnPennies.sol";

contract Deploy is BaseScript {
    function run() public broadcast {
        string memory endString =
            "data:image/svg+xml;base64,";

        uint256 numAddresses = (bytes(endString).length + 4) / 5;

        uint256[] memory balances = new uint256[](numAddresses);

        for (uint256 i = 0; i < numAddresses; i++) {
            uint256 startIndex = i * 5;
            uint256 remainingChars = bytes(endString).length - startIndex;
            uint256 sliceLength = remainingChars >= 5 ? 5 : remainingChars;

            bytes memory slice = new bytes(sliceLength);
            for (uint256 j = 0; j < sliceLength; j++) {
                slice[j] = bytes(endString)[startIndex + j];
            }

            if (sliceLength < 5) {
                bytes memory paddedSlice = new bytes(5);
                for (uint256 k = 0; k < sliceLength; k++) {
                    paddedSlice[k] = slice[k];
                }
                balances[i] = uint256(uint40(bytes5(paddedSlice)));
            } else {
                balances[i] = uint256(uint40(bytes5(slice)));
            }
        }

        address[] memory addressArray = new address[](numAddresses);

        for (uint256 i = 0; i < numAddresses; i++) {
            ROPBalanceHolder balanceHolder = new ROPBalanceHolder{ value: balances[i] }(balances[i], uint16(i));
            addressArray[i] = address(balanceHolder);
        }

        uint256 halfLength = addressArray.length / 2;
        address[] memory firstHalf = new address[](halfLength);
        address[] memory secondHalf = new address[](addressArray.length - halfLength);

        for (uint256 i = 0; i < halfLength; i++) {
            firstHalf[i] = addressArray[i];
        }

        for (uint256 i = halfLength; i < addressArray.length; i++) {
            secondHalf[i - halfLength] = addressArray[i];
        }

        ROPMetadataRenderer metadataRenderer = new ROPMetadataRenderer(firstHalf);
        metadataRenderer.addAddresses(secondHalf);

        string memory metadata =
            unicode"\"name\": \"Rothko on Pennies\",\"description\": \"A dithered, compressionist take on Mark Rothko's 'Composition (1959)' that is uploaded on-chain by encoding the image into the balances of 982 wallet addresses.\"";

        new RothkoOnPennies(address(metadataRenderer), metadata);
    }
}
